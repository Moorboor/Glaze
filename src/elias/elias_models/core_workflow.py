# Core notebook-first workflow for pooled model comparison on participant data.
# Main functions: prepare_modeling_data, fit_models_train_split,
# score_models_test_split, run_model_comparison.
...
def prepare_modeling_data(
    *,
    ...
    """Load, preprocess, attach subjective hazard, and build normative state columns."""
    ...
    _validate_required_columns(
        df_model,
        ("H", "prev_normative_belief_L", "normative_belief_L", "split"),
...
def fit_models_train_split(
    ...
    *,
    ...
    for idx, model_name in enumerate(model_names):
        model_seed = int(random_seed) + (idx * 1000)
        model_fit = fit_model_parameters(
            train_df,
            model_name=model_name,
            fit_config=fit_config,
            random_seed=model_seed,
        )
        fit_results[model_name] = model_fit
        fit_rows.append(
            {
                "model_name": str(model_name),
                "fit_objective": str(model_fit["fit_objective"]),
                "best_fit_objective_score": float(model_fit["best_fit_objective_score"]),
                "best_joint_score_train": float(model_fit["best_joint_score"]),
                "best_choice_only_score_train": float(model_fit["best_choice_only_score"]),
                "best_rt_only_cond_score_train": float(model_fit["best_rt_only_cond_score"]),
                "n_parameters": int(model_fit["n_parameters"]),
                "n_evaluations": int(model_fit["n_evaluations"]),
                "fit_seed": int(model_seed),
            }
        )

    fit_table = pd.DataFrame(fit_rows).sort_values(
        ["best_fit_objective_score", "model_name"], ascending=[True, True]
...
def score_models_test_split(
    ...
    *,
    ...
    """Score fitted models on pooled TEST rows."""
    ...
    for idx, model_name in enumerate(sorted(fitted_models.keys())):
        ...
        trial_scores_by_model[str(model_name)] = trial_scores
    ...
    score_table = pd.DataFrame(score_rows).sort_values(
        ["joint_score_test", "model_name"], ascending=[True, True]
    ).reset_index(drop=True)
...
def run_model_comparison(
    *,
    ...
    """Run pooled TRAIN fitting + pooled TEST scoring for all candidate models."""
    model_names = _normalize_candidate_models(candidate_models)
